...
function corp_post_Data($item, $order_id, $deedVersion, $constitutionVersion, $billingArray, $billingName, $billingCompany, $billing_phone, $product_id, $adviserEmail, $userID, $orderDate) {
		...
		$companyRegisteredOffice = array(			
				...
				'streetName' => $item['item_meta']['registeredOfficeStreet'][0], 
				'suburbName' => $item['item_meta']['registeredOfficeSuburb'][0],
				'stateName' => $item['item_meta']['registeredOfficeState'][0],
		...
		//Director 1 Details
		$d1TownOfBirth = $item['item_meta']['d1CityOfBirth'][0];
		...
		//Director 2 Details
		$d2TownOfBirth = $item['item_meta']['d2CityOfBirth'][0];
		...
		//Director 3 Details
		$d3TownOfBirth = $item['item_meta']['d3CityOfBirth'][0];
		...
		//Director 4 Details
		$d4TownOfBirth = $item['item_meta']['d4CityOfBirth'][0];
		...
		if ($sh1Type == "Individual"){
			...
			$sh1HasAcnOrArbn = $item['item_meta']['sh1HasAcnOrArbn'][0];
			if ($sh1HasAcnOrArbn == "Yes") {
		...
		if ($sh2Type == "Individual"){
			...
			$sh2HasAcnOrArbn = $item['item_meta']['sh2HasAcnOrArbn'][0];
			if ($sh2HasAcnOrArbn == "Yes") {
		...
		if ($sh3Type == "Individual"){
			...
			$sh3HasAcnOrArbn = $item['item_meta']['sh3HasAcnOrArbn'][0];
			if ($sh3HasAcnOrArbn == "Yes") {
				$sh3Acn = $item['item_meta']['sh3Acn'][0];
			}
			else {
				$sh3Acn = NULL;
			}
		...
		if ($sh4Type == "Individual"){
			...
			$sh4HasAcnOrArbn = $item['item_meta']['sh4HasAcnOrArbn'][0];
			if ($sh4HasAcnOrArbn == "Yes") {
		...
		$t = $item['item_meta']['numExtraShareholders'][0];
		...
		$newCompanyArray = array (
				...
				'D3Details' => $d3details,
				'D4Details' => $d4details,
				'numShareholders' => $item['item_meta']['numExtraShareholders'][0],
